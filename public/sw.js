  ...
  if (request.mode === 'navigate') {
    event.respondWith(
      fetch(request)
        ...
        .catch(async () => {
          // Each branch has to be awaited: caches.match() returns a promise,
          // which is truthy even when it resolves to nothing, so the last two
          // fallbacks were unreachable and a miss answered with `undefined`.
