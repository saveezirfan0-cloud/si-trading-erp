...
import { Btn, Input, Select, Textarea, Card, FormGrid, ItemPicker } from '../../components/ui';
...
export default function PurchaseInvoiceForm({ invoice, onBack, onPreview }) {
  ...
  useEffect(() => {
    const load = async () => {
      const [s, inv, pi] = await Promise.all([getAll(COLLECTIONS.SUPPLIERS), getAll(COLLECTIONS.INVENTORY), getAll(COLLECTIONS.PURCHASE_INVOICES)]);
