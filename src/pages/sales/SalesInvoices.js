...
import { Plus, Edit2, Trash2, Download, Eye, FileText, TrendingUp, ClipboardCheck, Paperclip } from 'lucide-react';
import {
...
export default function SalesInvoices() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await remove(COLLECTIONS.SALES_INVOICES, id); toast.success('Moved to trash'); }
    catch (e) { toast.error(e.message); }
  };

  // Drafts and invoices still under review are not revenue and not owed yet.
  const totalRevenue = invoices.filter(i => countsToTotals(i.status)).reduce((s, i) => s + (i.total || 0), 0);
  ...
  // Keep the open record in step with realtime updates (an approval made in
  // another tab, say) instead of showing the snapshot taken when it was opened.
  ...
  const columns = [
    ...
    { key: 'items', label: 'Items', render: (v, r) => (
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <Badge color="blue">{v?.length || 0} items</Badge>
        {r.attachments?.length > 0 && (
          <span title={`${r.attachments.length} attachment(s)`} style={{ display: 'inline-flex', color: 'var(--text3)' }}>
            <Paperclip size={13} />
          </span>
        )}
      </div>
    ...
    { key: '_actions', label: '', render: (_, row) => (
      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
        ...
        <HistoryButton collection={COLLECTIONS.SALES_INVOICES} recordId={row.id} label={row.invoiceNo} />
